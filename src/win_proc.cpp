...
WinProcess Constructor::create(std::wstring_view exe_path, std::wstring console_title)
{
    WinProcess process {};
    ...
    BOOL rs = CreateProcessW(
            ...
            m_cmd_args.empty() ? nullptr : m_cmd_args.data(),
            nullptr, nullptr, false, // Не меняем параметры безопасности
            m_startup_flags ? m_startup_flags : NORMAL_PRIORITY_CLASS,
            m_env_var.empty() ? nullptr : reinterpret_cast<void*>(m_env_var.data()),
            m_start_dir.empty() ? nullptr : m_start_dir.data(),
            &m, &process.m_info);
    if (rs == 0) {
        ...
        throw WinError("Failed to create WinProcess with title: " + narrow_title, GetLastError());
    }
    return process;
}
...
Constructor& Constructor::set_window_size_pxl(DWORD width, DWORD height) noexcept
{
    m.dwXSize = width; m.dwYSize = height;
    m.dwFlags |= STARTF_USESIZE;
    return *this;
}

Constructor& Constructor::set_console_size_chr(DWORD width, DWORD height) noexcept
{
    m.dwXCountChars = width; m.dwYCountChars = height;
    m.dwFlags |= STARTF_USECOUNTCHARS;
    return *this;
}

Constructor& Constructor::set_console_color(ConsoleColor foreground, ConsoleColor background) noexcept
{
...
Constructor& Constructor::set_window_show_parameter(int show_parameter) noexcept
{
...
HANDLE WinProcess::get_handle() noexcept
{
    return m_info.hProcess;
}

DWORD WinProcess::get_id() const noexcept
{
    return m_info.dwProcessId;
}

HANDLE WinProcess::get_thread_handle() noexcept
{
    return m_info.hThread;
}

DWORD WinProcess::get_thread_id() const noexcept
{
    return m_info.dwThreadId;
}

} // namespace windows_wrappers
