...
struct BindInstanceSt
{
  // Параметр шаблона - последовательность индексов для прохода по кортежу.
  // Получили количество индексов равное количеству аргументов через std::index_sequence_for
  template <size_t... Inds>
  static std::function<ReturnTy(Args...)> bind(ClassTy* instance,
          ReturnTy (ClassTy::*method) (Args...), std::index_sequence<Inds...>)
  {
...
template <class ClassTy, class ReturnTy, class ... Args>
std::function<ReturnTy(Args...)> bind_instance(ClassTy* instance, ReturnTy (ClassTy::*method) (Args...))
{
...
template<class Ty>
typename std::enable_if<sizeof(Ty) % 2 != 1, std::wstring>::type cmd_arg_from(const Ty& data)
{
  // Диапазон символов [0, 32], не используемых в аргументах командной строки
  // 32 - символ пробела - первый печатный символ - разделяет аргументы командной строки
...
// Создает объект из аргумента командой строки, полученного методом cmd_arg_from.
// Вторым элементом возвращает позицию символа, на котором завершилось преобразование
template<class Ty>
std::pair<Ty, size_t> cmd_arg_to(std::wstring_view arg_str)
{
  ...
  size_t j = 0;
  for (size_t i = 0; arg_str[j] != L'\0' && arg_str[j] != L' ' && i < sizeof(Ty) / sizeof(wchar_t); ++i, ++j) {
