...
// event_dispatcher.add_key_handling(Hotkey('A', Hotkey::Modifier::alt), handler));
// Удобнее пользоваться литералом
...
class Hotkey
{
    ...
    : m_key_code{static_cast<UINT>(key_code)}, m_modifiers{modifiers}
    {
        if (! validate_key_code(key_code)) {
            throw std::runtime_error("Hotkey c-tor acceptable key codes are chars from [A, Z]");
        } 
    }

    constexpr UINT get_key_code()const noexcept
    {
        return m_key_code;
    }

    constexpr Modifier get_modifiers() const noexcept
    {
        return m_modifiers;
    }
    ...
    {   {"alt", Hotkey::Modifier::alt},
        {"ctrl", Hotkey::Modifier::ctrl},
        {"shift", Hotkey::Modifier::shift},
        {"win", Hotkey::Modifier::win},
...
constexpr Hotkey::Modifier operator|(Hotkey::Modifier left, Hotkey::Modifier right) noexcept
{
    return static_cast<Hotkey::Modifier>(static_cast<UINT>(left) | static_cast<UINT>(right));
}
