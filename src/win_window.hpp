...
public:
  ...
  public:
    ...
    enum class MixBehavior : unsigned char {
      ...
      combine = 1, // параметры не стираются, а дополяются, если это возможно
                   // (Исходные данные в приоритете).
    ...
    WinWindow create(HINSTANCE app_hinstance,
    ...
    Constructor &set_clss_style(UINT style,
                                MixBehavior b = MixBehavior::combine);
    // https://learn.microsoft.com/en-us/windows/win32/winmsg/window-styles
    Constructor &set_wnd_style(DWORD style,
                               MixBehavior b = MixBehavior::combine);
    // https://learn.microsoft.com/en-us/windows/win32/winmsg/extended-window-styles?source=recommendations
    Constructor &set_wnd_exstyle(DWORD style,
                                 MixBehavior b = MixBehavior::combine);
    // В координатах экрана
    Constructor &set_position(int x, int y,
                              MixBehavior b = MixBehavior::rewrite);
    // Размер в пикселях
    Constructor &set_size(int width, int height,
                          MixBehavior b = MixBehavior::rewrite);
    // Позволяет обработать сообщения, возникающие, в том числе, при создании
    // окна (WM_CREATE)
    Constructor &add_message_handling(UINT message_code, EventHandler handler,
                                      MixBehavior b = MixBehavior::combine);
  ...
  private:
    template <class Ty>
    void mix_flags(Ty &my_flag, MixBehavior &my_b, Ty other_flag,
                   MixBehavior other_b) {
      if (my_b == MixBehavior::not_set || other_b == MixBehavior::rewrite) {
        my_b = other_b;
        my_flag = other_flag;
      } else if (my_b == MixBehavior::combine &&
                 other_b == MixBehavior::combine) {
        my_flag |= other_flag;
      }
    }
    template <class Ty>
    void mix_fields(Ty &my_field, MixBehavior &my_b, Ty other_field,
                    MixBehavior other_b) {
      if (my_b == MixBehavior::not_set || other_b == MixBehavior::rewrite) {
        my_b = other_b;
...
private:
  ...
  int m_id{0};
  // Храним указатели, а не сами объекты - после перемещения адреса должны
  // сохранятся
