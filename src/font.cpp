...
int font_search_success(const LOGFONTA *font_criteria,
                        const TEXTMETRICA *metric, DWORD font_type,
                        LPARAM success_flag_ptr) {
...
bool is_font_available(std::string_view font_name) {
  ...
  // Из MSDN: Must be set to zero for all language versions of the operating
  // system.
  font_criteria.lfPitchAndFamily = 0;

  bool success_flag{false};
  EnumFontFamiliesExA(global_dc, &font_criteria, font_search_success,
                      reinterpret_cast<LPARAM>(&success_flag), {});
  return success_flag;
}

HKEY open_con_font_hkey(DWORD &res_code, REGSAM access_rights) {
  static constexpr const char *subkey =
      "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Console\\TrueTypeFont";
  HKEY hkey{nullptr};
  res_code = RegOpenKeyExA(HKEY_LOCAL_MACHINE, subkey, 0, access_rights, &hkey);
  return hkey;
}
...
std::string read_registry_string(HKEY hkey, const char *value_name,
  ...
  ret_code = RegGetValueA(hkey, 0, value_name,
                          2, // PRF_RT_REG_SZ
                          0, buf, &data_size);
