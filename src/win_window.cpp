...
WinWindow WinWindow::Constructor::create(HINSTANCE app_hinstance,
  ...
  *g_logger << "Window class with id " + std::to_string(class_id) +
  ...
  auto destroy_handler =
      [quit_flag_lambda](const MSG &msg) noexcept -> LRESULT {
    PostQuitMessage(0);
    quit_flag_lambda->store(true);
    return 0;
  };
  wnd.m_destroy_handler = EventHandlerOwner(destroy_handler);
  event_dispatcher.add_message_handling(class_id, WM_DESTROY,
                                        wnd.m_destroy_handler.get());
...
WinWindow::Constructor &WinWindow::Constructor::set_show_flag(int flag,
                                                              MixBehavior b) {
  mix_fields(m_show_flag, m_param_mixb.show, flag, b);
  return *this;
}

WinWindow::Constructor &WinWindow::Constructor::set_clss_style(UINT style,
                                                               MixBehavior b) {
  mix_flags(m_clss_style, m_param_mixb.class_style, style, b);
  return *this;
}

WinWindow::Constructor &WinWindow::Constructor::set_wnd_style(DWORD style,
                                                              MixBehavior b) {
  mix_flags(m_wnd_style, m_param_mixb.wnd_style, style, b);
  return *this;
}

WinWindow::Constructor &WinWindow::Constructor::set_wnd_exstyle(DWORD style,
                                                                MixBehavior b) {
  mix_flags(m_wnd_exstyle, m_param_mixb.wnd_exstyle, style, b);
  return *this;
}

WinWindow::Constructor &WinWindow::Constructor::set_position(int x, int y,
                                                             MixBehavior b) {
...
WinWindow::Constructor &WinWindow::Constructor::set_size(int width, int height,
                                                         MixBehavior b) {
...
WinWindow::Constructor &WinWindow::Constructor::add_message_handling(
    UINT message_code, EventHandler handler, MixBehavior b) {
  if (m_param_mixb.handlers == MixBehavior::not_set ||
      b == MixBehavior::rewrite) {
...
WinWindow::Constructor &WinWindow::Constructor::mix_with(Constructor other) {
  ...
  if (m_param_mixb.handlers == MixBehavior::not_set ||
    ...
    m_msg_handlers.insert(m_msg_handlers.end(),
...
void WinWindow::construct(HINSTANCE app_hinstance,
                          EventDispatcher &event_dispatcher) {
...
WinWindow::WinWindow(WinWindow &&other) noexcept
    : m_hwnd{other.m_hwnd}, m_dispatcher{other.m_dispatcher}, m_id{other.m_id},
...
int WinWindow::get_id() const noexcept { return m_id; }

bool WinWindow::is_quit() const noexcept { return mpf_quit->load(); }

HWND WinWindow::get_hwnd() noexcept { return m_hwnd; }

LRESULT WinWindow::procedure_wrapper(HWND hwnd, UINT message, WPARAM wparam,
                                     LPARAM lparam) {
