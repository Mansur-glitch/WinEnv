/* Утилита для создание переносимой среды разработки в Windows 7 и старше
 * Автор: Мухаметзянов Мансур
 * Обновлен: 11.09.24
 * Функионал:
 *  Вызов консоли сочетанием клавиш;
 *  Для консоли:
 *    Добавляются переменные среды, расширяется переменная PATH;
 *    Применяется кастомизация (размер и положение окна, шрифт, цвета);
 *  Открытие файлов текстовым редактором nvim "перетягиванием" файлов;
 *  Открытие вкладки браузера chromium с адресом из буфера обмена;
...
int WINAPI wWinMain(HINSTANCE hinstance, HINSTANCE hPrevInstance,
  ...
  if (cmdline.empty()) {
    ...
    if (!log_file.good()) {
      MessageBoxA(nullptr,
                  "Couldn't open log.txt\n Process is probably already "
                  "started\nUse HK_EXIT to finish it",
  ...
  try {
    ...
    if (!cmdline.empty()) {
      ...
      if (cmdline.substr(0, 4) == L"font") {
        ...
        RegCloseKey(hkey);
        if (ret_code != ERROR_SUCCESS) {
          throw WinError("Failed to write console font registry value",
                         ret_code);
        } else {
          MessageBoxA(
              nullptr,
              "Registry value with specified console font was successfully set",
