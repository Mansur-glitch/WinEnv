    ...
    : m_config{"config.json"},
      ...
      m_log_wnd{m_hinstance,
                ...
                WinWindow::Constructor()
                    ...
                    .add_message_handling(
                        WM_LBUTTONDBLCLK,
                        method_handle(&RootApp::log_wnd_2clk_handler))
                    .add_message_handling(
                        WM_NCLBUTTONDBLCLK,
                        method_handle(&RootApp::log_wnd_2clk_handler)),
                "",
      ...
      m_file_wnd{
          ...
          WinWindow::Constructor()
              ...
              .set_size(200, 200)
              .add_message_handling(
                  WM_DROPFILES, method_handle(&RootApp::file_drop_msg_handler))
              .add_message_handling(WM_PAINT,
                                    method_handle(&RootApp::paint_file_wnd))},
...
RootApp::~RootApp() {
  *g_logger << "Root app is being destructed" << std::endl;
  if (mf_added_fonts) {
    manage_font_resouces<FontAction::remove>(std::filesystem::current_path() /
                                             "fonts");
  }
...
void RootApp::run() {
  try {
    ...
    std::string log_msg =
        std::string(log_text_top) + ex.what() + log_text_bottom;
...
std::string RootApp::configure_hotkeys() {
  ...
  auto add_key_handling_with_backup = [this, &log_msg](Hotkey hk,
                                                       auto handler) -> void {
    static constexpr Hotkey::Modifier backup_modifier =
        Hotkey::Modifier::ctrl | Hotkey::Modifier::win | Hotkey::Modifier::alt |
    ...
    try {
      ...
      try {
        ...
        log_msg += "Failed to register \"" + hk.to_stdstring() +
                   "\" hotkey. \"" + backup_hk.to_stdstring() +
                   "\" used instead.\n";
  ...
  add_key_handling_with_backup(m_config.exit_hk,
                               method_handle(&RootApp::exit_khandler));
  add_key_handling_with_backup(m_config.spawn_cmd_hk,
...
void RootApp::add_con_font_to_registry(std::string font_name) {
  ...
  std::string prev_font =
      read_registry_string(hkey, g_confont_reg_valname, ret_code);
  RegCloseKey(hkey);
  ...
  ret_code = RegSetValueEx(hkey, g_confont_reg_valname, 0, REG_SZ,
                           reinterpret_cast<BYTE *>(font_name.data()),
                           font_name.size() + 1);
  RegCloseKey(hkey);
  if (ret_code != ERROR_SUCCESS) {
...
void RootApp::create_child_console(std::wstring_view launch_command) {
  ...
  std::copy(m_config.term_color_table.begin(), m_config.term_color_table.end(),
            console_app_args.term_colors);
  std::copy(m_config.font_name.begin(), m_config.font_name.end(),
            console_app_args.font_name);
  console_app_args.font_size = m_config.font_size;
  // В командной строку WinMain входит последовательность символов после первого
  // пробела
  ...
  WinProcess proc =
      WinProcess::Constructor()
          ...
          .add_startup_flags(
              CREATE_NEW_CONSOLE) // По флагу отличаем дочерний процесс
...
LRESULT RootApp::paint_file_wnd(const MSG &msg) {
  ...
  std::array ico_vetrexes{
      POINT{cx - 3 * dp, cy - 4 * dp}, POINT{cx + 1 * dp, cy - 4 * dp},
      POINT{cx + 1 * dp, cy - 2 * dp}, POINT{cx + 3 * dp, cy - 2 * dp},
      POINT{cx + 3 * dp, cy + 4 * dp}, POINT{cx - 3 * dp, cy + 4 * dp},
      POINT{cx - 3 * dp, cy - 4 * dp},

      POINT{cx + 1 * dp, cy - 4 * dp}, POINT{cx + 3 * dp, cy - 2 * dp},

      POINT{cx - 2 * dp, cy - 1 * dp}, POINT{cx + 2 * dp, cy - 1 * dp},

      POINT{cx - 2 * dp, cy},          POINT{cx + 2 * dp, cy},

      POINT{cx - 2 * dp, cy + 1 * dp}, POINT{cx, cy + 1 * dp},
  };
  std::array<DWORD, 5> connected_vertexes = {7, 2, 2, 2, 2};
  PolyPolyline(hdc, ico_vetrexes.data(), connected_vertexes.data(),
               connected_vertexes.size());
