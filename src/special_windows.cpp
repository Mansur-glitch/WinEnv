...
BorderlessWindow::BorderlessWindow(HINSTANCE app_hinstance,
  ...
  :
  WinWindow{wnd_config
    .add_message_handling(WM_NCCALCSIZE, method_handle(&BorderlessWindow::nccalcsize))
    .add_message_handling(WM_NCHITTEST, method_handle(&BorderlessWindow::nchittest))
...
{
  EventDriven::reasign_owner(this);
  delayed_construction(WinWindow::Constructor()    
    .add_message_handling(WM_NCCALCSIZE, method_handle(&BorderlessWindow::nccalcsize))
    .add_message_handling(WM_NCHITTEST, method_handle(&BorderlessWindow::nchittest))
  );
}

LRESULT BorderlessWindow::nccalcsize(const MSG& msg)
{
  return 0;
}

LRESULT BorderlessWindow::nchittest(const MSG& msg)
{
  return HTCAPTION;
}
...
HiddenWindow::HiddenWindow(HINSTANCE app_hinstance,
  ...
  WinWindow::Constructor wnd_config)
  : 
  m_timer_id{g_rand_gen() % 100'001},
  mf_shown{is_shown_initialy},
  WinWindow{wnd_config
...
bool HiddenWindow::is_shown() const noexcept
{
  return mf_shown;
}
