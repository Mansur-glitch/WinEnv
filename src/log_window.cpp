...
RECT get_text_area(std::string_view text, HFONT font)
{
  ...
  int text_height = DrawTextA(hdc, text.data(), text.size(), &text_area, DT_CALCRECT);
  if (text_height == 0) {
    text_area = {0, 0, 0, 0};
  }
  if (font != nullptr) {
...
RECT get_draw_area(RECT initial_area, int border_width) {
  ...
  initial_area.right = initial_area.right > screen_width ? screen_width : initial_area.right;
  initial_area.bottom = initial_area.bottom > screen_height ? screen_height : initial_area.bottom;
...
LogWindowFields::LogWindowFields(std::string text, std::string font_name, size_t font_size)
: m_text{std::move(text)}, m_font_name{std::move(font_name)}
{
  if (! m_font_name.empty()) {
    ...
    CLIP_DEFAULT_PRECIS,
    CLEARTYPE_QUALITY,
    FIXED_PITCH | FF_MODERN, // моноширинный шрифт предпочтительнее
    m_font_name.c_str());
  }
...
LRESULT LogWindow::paint(const MSG &msg)
{
  ...
  RECT text_area {m_border_width, m_border_width,
       border_area.right - m_border_width, border_area.bottom - m_border_width};
  HFONT original_font {nullptr};
  if (m_font != nullptr) {
