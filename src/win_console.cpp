...
namespace winenv 
{
  ...
  HWND WinConsole::get_attached_console_hwnd()
  {
      ...
      if (! SetConsoleTitleA(temp_con_title_n)) {
        throw WinError("Failed to set temporary console window title", GetLastError());
      };
      ...
      if (! SetConsoleTitleW(old_con_title)) {
        throw WinError("Failed to reset original console window title", GetLastError());
      }
      return(hwnd);
  }
  ...
  if (search_console_title.empty()) {
    ...
    if (! m_hwnd) {
      throw WinError(std::string{"Failed to find console window with title: "}
         + narrow_string(search_console_title), GetLastError());
    }
  }
...
const std::unique_ptr<WinConsole>& WinConsole::get()
{
  return s_instance;
}
...
void WinConsole::allocate(std::wstring_view search_console_title)
{
  if (s_instance != nullptr) {
    ...
    s_instance = std::unique_ptr<WinConsole>(new WinConsole(search_console_title));
  }
}
...
void WinConsole::resize_chrs(short width, short height, short buf_width, short buf_height)
{
  static constexpr short n_console_buf_rows = 500;

  // Нужно убедится, что буфер консоли больше размера отображаемого символьного окна
  if (buf_width == 0) {
    buf_width = width;
    buf_height = height + n_console_buf_rows;
  } else if (buf_height == 0) {
    if (buf_width < width) {
      throw std::runtime_error("Failed to resize console. Buffer size must be greater than window size");
    }
    buf_height = height + n_console_buf_rows; 
  ...
  if (! SetConsoleScreenBufferSize(m_console_out_handle, {buf_width, buf_height})){
    throw WinError("Failed to set console number columns and rows", GetLastError());             
  }
  ...
  con_win_rc.Right = static_cast<short>(std::min(width - 1,
              screen_buf_info.dwMaximumWindowSize.X - 1));
  con_win_rc.Bottom = static_cast<short>(std::min(height -1,
              screen_buf_info.dwMaximumWindowSize.Y - 1));
  if (! SetConsoleWindowInfo(m_console_out_handle, true, &con_win_rc)) {
    throw WinError("Failed to set console number columns and rows", GetLastError());             
  }
  // Обновляем информацию о буфере консоли для корректной работы зависящих методов
...
void WinConsole::set_font(std::string_view font_name, short font_height)
{
    ...
    font_info.dwFontSize = {0, font_height}; // Ширина будет подобрана автоматически
...
void WinConsole::configure_use_startup_info(const STARTUPINFOW& start_info)
{
    if (start_info.dwFlags & STARTF_USECOUNTCHARS) {
        ...
        if (! SetConsoleTextAttribute(m_console_out_handle, start_info.dwFillAttribute)) {
          throw WinError("Failed to configure console use startup info", GetLastError());
        }
    }
}
