...
namespace {
  // Вспомогательная функция. Вызывает обработчики, привязанные к сочетанию клавиши или к сообщению
  template<class Iter>
  LRESULT call_handlers_in_range(Iter beg, Iter end,
...
: mp_handler{std::move(other.mp_handler)}, mpf_alive{std::move(other.mpf_alive)}
{}
...
void EventHandlerOwner::set_alive(bool f_alive)
{
  if (mpf_alive != nullptr) {
    *mpf_alive = f_alive;
    ...
    throw std::runtime_error("Failed to set_alive to EventHandlerOwner. It was empty (default constructed)");
  }
}
...
HotkeyId get_registered_key_id(Hotkey hk)
{
  ...
  if (auto iter = s_registered_keys.find(hk); iter != s_registered_keys.end()) {
    ...
    if (RegisterHotKey(NULL, new_key_id,
      static_cast<UINT>(hk.get_modifiers()), hk.get_key_code()))
    {
      s_registered_keys.insert({hk, new_key_id});
      return new_key_id;
    } else {
      throw WinError("Couldn't register <" + hk.to_stdstring() +"> key combination", GetLastError());
    }
  }
}
...
void EventDispatcher::add_message_handling(UINT message_code, EventHandler handler)
{
    m_handlers.push_back(handler);
...
void EventDispatcher::add_message_handling(int window_id, UINT message_code, EventHandler handler)
{
  m_handlers.push_back(handler);
...
void EventDispatcher::dispatch(std::pair<UINT, UINT> msg_filter, HWND wnd_filter)
{
    MSG msg = {};
    ZeroMemory(&msg, sizeof(msg));
    // Извлекает полученные на данный момент сообщения, удовлетворяющие заданные параметры
    while (PeekMessageA(&msg, wnd_filter, msg_filter.first, msg_filter.second, PM_REMOVE) != 0) {
        ...
        } else if (msg.hwnd == nullptr) { // Сообщение адресовано потоку, а не конкретному окну
          UINT message_id = msg.message;
          // Возможно несколько обработчиков относится к одному идентификатору сообщения
          auto [iter, end_iter] = m_msg_bindings.equal_range({message_id, WinWindow::window_id_thread});
          if (iter != end_iter) {
            call_handlers_in_range(iter, end_iter, m_handlers, msg);
          }
        } 
        TranslateMessage(&msg); // Для обработки нажатий клавиш как символьных сообщений
        DispatchMessageA(&msg); // Отправляет на обработку оконной процедуре window_procedure
    }
}

LRESULT EventDispatcher::window_procedure(HWND hwnd, UINT message_id,
                                                   WPARAM wparam, LPARAM lparam, int window_id)
{
    MSG msg {};
    ...
    auto [iter1, end_iter1] =  m_msg_bindings.equal_range({message_id, WinWindow::window_id_thread});
    if (iter1 != end_iter1) {
