...
class RootApp : public HasEventDispatcher, public EventDriven
{
  ...
  {
    public:
    inline FileDropWnd(HINSTANCE app_hinstance, EventDispatcher& event_dispatcher,
                WinWindow::Constructor wnd_config = {})
    {
      delayed_construction(wnd_config
        .set_wnd_exstyle(WS_EX_ACCEPTFILES));
      construct(app_hinstance, event_dispatcher);
    }
  };

  public:
  // Данные, которые передаются дочернему процессу через аргумент командной строки
  struct CmdLineArg
  {
  ...
  // Создает дочерний процесс с настроенной консолью, в которой запускается указанная команда
  ...
  static constexpr const char* log_text_top =     "............Info............\n\n";
